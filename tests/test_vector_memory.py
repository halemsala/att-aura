...
class TestVectorMemory(unittest.TestCase):
    ...
    def test_dim_migration_reindexes(self):
        ...
        mem8.index_case(make_features(minute=60), outcome="win")
        mem8.index_case(make_features(minute=61), outcome="loss")
        mem8.close()
        mem16 = self._mem(provider=StaticTestProvider(dim=16), min_cases=2)
        mem16.index_case(make_features(minute=62), outcome="win")
