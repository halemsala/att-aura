    ...
    def search(self, features: Dict, k: Optional[int] = None) -> SearchResult:
        ...
        try:
            ...
            with self._lock:
                ...
                try:
                    rows = conn.execute(
                        "SELECT case_id, distance FROM vec_cases "
                        f"WHERE embedding MATCH ? AND k = {kq_safe} ORDER BY distance",
                        (blob,)).fetchall()
                except sqlite3.OperationalError as e:
                    LOG.warning("RAG: erro na query vetorial: %s", e)
                    return SearchResult(status="error", note=f"vec_query_error:{e}",
                                        n_indexed=total, provider=pname)
                if not rows:
                    return SearchResult(status="insufficient_evidence", note="no_cases",
                                        n_indexed=total, provider=pname)
                ids = [int(r["case_id"]) for r in rows]
                dist = {int(r["case_id"]): float(r["distance"]) for r in rows}
                qmarks = ",".join("?" * len(ids))
